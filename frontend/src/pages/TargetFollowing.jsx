...
function TargetFollowing() {
  ...
  const updateDB = async (hoverTime) => {
    try {
      const res = await axios.post(
        "http://localhost:5000/update/target-following",
        // CHANGE: Send the final score as 'hoverTime' and ensure it's in seconds (string)
        { username, hoverTime: (hoverTime / 1000).toFixed(2) }, // Use 'hoverTime' as the key
        { withCredentials: true }
      );
    } catch (err) {
      console.error("ASDKLASJDKLA");
    }
  };
  ...
  const startGame = useCallback(() => {
    ...
    speedTimerRef.current = setInterval(() => {
      ...
      setMessage(
        `Speed increased! Current speed: ${Math.abs(target.current.vx).toFixed(
          2
        )}`
      );
